...
if [ "$(uname)" == "Linux" ]; then
        echo "Starting elevatorserver with UDP port: $elevator_server_udp_port" 
	elevatorserver --port $elevator_server_udp_port &
        sleep 1
        echo "Starting elevator using UDP port: $udp_port ID: $id" 
	cd $go_program_path
	./distributedElevator --port $udp_port --id $id
else
