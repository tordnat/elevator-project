...
func TestConsensusBarrier(t *testing.T) {
	...
	orderSys.CabRequests["0"][0].OrderState["0"] = unconfirmedOrder
	orderSysAfterTrans := requestSync.ConsensusBarrierTransition("0", orderSys)
	if orderSysAfterTrans.CabRequests["0"][0].OrderState["0"] != confirmedOrder {
		t.Error("Failed assert, did not barrier transition cab correct")
	}
	if orderSysAfterTrans.CabRequests["0"][1].OrderState["0"] != unknownOrder {
		t.Error("Failed assert, transitioned unknown order to", orderSysAfterTrans.CabRequests["0"][1].OrderState["0"])
	}

	orderSys = requestSync.NewSyncOrderSystem("0")
	//Set floor zero cab req to unknown
	orderSys.CabRequests["0"][0].OrderState["0"] = confirmedOrder
	orderSysAfterTrans = requestSync.ConsensusBarrierTransition("0", orderSys)
	if orderSysAfterTrans.CabRequests["0"][0].OrderState["0"] != confirmedOrder {
	...
	orderSys = requestSync.NewSyncOrderSystem("0")
	orderSys.CabRequests["0"][0].OrderState["0"] = servicedOrder
	orderSys.CabRequests["0"][0].OrderState["1"] = servicedOrder
	orderSysAfterTrans = requestSync.ConsensusBarrierTransition("0", orderSys)
	if orderSysAfterTrans.CabRequests["0"][0].OrderState["0"] != noOrder {
		t.Error("Cab order should be completed after transitioning got: ", orderSysAfterTrans.CabRequests["1"][0].OrderState["0"])
	}
	if orderSysAfterTrans.CabRequests["0"][1].OrderState["0"] != unknownOrder {
